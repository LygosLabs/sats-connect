...
import { BitcoinNetwork, BitcoinNetworks, chainHashFromNetwork } from 'bitcoin-network';
import {
  address,
 ...
 */
export class NewBitcoinSatsConnectProvider extends Provider implements Partial<WalletProvider> {
  private wallet: WalletInterface;
  private network: BitcoinNetwork;
  private _ddk?: DdkInterface;

  constructor(options: BitcoinSatsConnectProviderOptions & { ddk?: DdkInterface }) {
    super();

    if (!options.wallet) {
      throw new Error('wallet is required - pass a WalletInterface implementation');
    }
  ...
  async createDlcOffer(
    ...
    fixedInputs?: { txid: string; vout: number; value: number; txHex?: string }[],
  ): Promise<DlcOffer> {
    try {
      ...
      if (!paymentAddress.publicKey) {
        throw new Error('Payment address does not have a public key');
      }
      ...
      dlcOffer.payoutSpk = address.toOutputScript(paymentAddress.address, this.network);
      ...
      dlcOffer.fundingInputs = await Promise.all(
        fixedInputs.map(async (input, index) => {
          ...
          if (!txHex) {
            txHex = (await this.getMethod('getRawTransactionByHash')(input.txid)) as
              | string
              | undefined;
          }

          if (!txHex) {
            throw new Error(`Failed to get transaction hex for txid: ${input.txid}`);
          }
      ...
      dlcOffer.changeSpk = address.toOutputScript(paymentAddress.address, this.network);
  ...
  private createP2MSMultisig(pubkey1: Buffer, pubkey2: Buffer) {
    const orderedPubkeys =
      Buffer.compare(pubkey1, pubkey2) === -1 ? [pubkey1, pubkey2] : [pubkey2, pubkey1];
  ...
  private getPayoutsFromEnumeratedDescriptor(
    ...
    payouts: { offer: bigint; accept: bigint }[];
    messagesList: Messages[];
  } {
    const payouts: { offer: bigint; accept: bigint }[] = [];
  ...
  private getPayoutsFromContractInfo(dlcOffer: DlcOffer): {
    ...
    if (contractInfo.contractInfoType === ContractInfoType.Single) {
      ...
      if (contractDescriptor.contractDescriptorType === ContractDescriptorType.Enumerated) {
    ...
    throw new Error('Only SingleContractInfo with EnumeratedDescriptor is supported');
  }
  ...
  async signDlcAccept(
    ...
    try {
      ...
      allFundingInputs.sort((a, b) => Number(a.inputSerialId) - Number(b.inputSerialId));
      ...
      const p2ms = this.createP2MSMultisig(dlcOffer.fundingPubkey, dlcAccept.fundingPubkey);
      ...
      const cetsForDdk = dlcTransactions.cets.map((cet) => this.convertTxToDdkTransaction(cet));
      ...
      const signResponse = await this.wallet.request<SignDlcResult>('dlc_signOffer', params);
      ...
      // Cast to the expected type - FundingSignatures expects ScriptWitnessV0[][]
      (fundingSignatures as { witnessElements: ScriptWitnessV0[][] }).witnessElements =
        witnessElements;
      ...
      const cetSigs: { encryptedSig: Buffer; dleqProof: Buffer }[] = [];
      ...
      (cetAdaptorSignatures as { sigs: { encryptedSig: Buffer; dleqProof: Buffer }[] }).sigs =
        cetSigs;
  ...
  async signCetForExecution(
    ...
    const signResponse = await this.wallet.request<{ psbt: string }>('signPsbt', {
      psbt: cetPsbt.toBase64(),
      signInputs: {
...
export default NewBitcoinSatsConnectProvider;
